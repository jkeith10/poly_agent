...
class MarketPriceRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint("yes_price >= 0 AND yes_price <= 1", name="ck_price_yes_probability"),
        CheckConstraint("no_price >= 0 AND no_price <= 1", name="ck_price_no_probability"),
        UniqueConstraint("market_id", "observed_at", name="uq_market_price_observation"),
        Index("ix_market_prices_market_observed", "market_id", "observed_at"),
    )
    id: Mapped[UUID] = mapped_column(primary_key=True, default=uuid4)
...
class RecommendationRecord(Base):
    ...
    market_probability: Mapped[Decimal] = mapped_column(Numeric(12, 10))
    edge: Mapped[Decimal] = mapped_column(Numeric(12, 10))
    expected_value: Mapped[Decimal] = mapped_column(Numeric(12, 10))
    fractional_kelly: Mapped[Decimal] = mapped_column(Numeric(12, 10))
    suggested_position: Mapped[Decimal] = mapped_column(Numeric(24, 8))
